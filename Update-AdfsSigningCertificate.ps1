...
Import-Module .\ADFS\ADFSDiagnostics.psm1 -ErrorAction SilentlyContinue -ErrorVariable moduleError
...
if ($null -ne $newCert.Thumbprint   -and -not $adfsProvider.SigningCertificate.Equals($newCert)) {
    # Do we need to add the new cert as CA in SharePoint?
    if ( $null -eq   (Get-SPTrustedRootAuthority | ? {$_.Certificate.Thumbprint -eq $newCert.Thumbprint}) ) {
